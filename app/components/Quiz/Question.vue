<template>
  <div class="p-4 bg-white rounded-lg">
    ...
    >
      ...
      <div v-if="question.type === 'multiple-choice'" class="multiple-choice">
        <fieldset :aria-labelledby="`question-${question.questionId}-text`">
          ...
          >
            <QuizCheckboxOption
              :input-id="`checkbox-${question.questionId}-${answer.answerId}`"
              :aria-labelledby="`answer-text-${question.questionId}-${answer.answerId}`"
              :model-value="selectedAnswersMap[answer.answerId] || false"
              @update:model-value="(value) => updateCheckbox(answer.answerId, value)"
            >
              <template #text>
                {{ answer.answerText }}
              </template>
            ...
            <label :for="`radio-${question.questionId}-${answer.answerId}`" class="w-full block">
              ...
              >
                <RadioButton
                  ...
                  :input-id="`radio-${question.questionId}-${answer.answerId}`"
          ...
          <div class="flex justify-center gap-4 mt-6">
            <div>
              ...
              >
                <RadioButton
                  v-model="selectedAnswer"
                  :input-id="`boolean-true-${question.questionId}`"
                  :name="`question-${question.questionId}`"
                  value="true"
                  class="hidden me-4"
                />
                <i class="pi pi-check-circle text-emerald-600 mr-2" /> Yes
              </label>
            </div>
            ...
            <div>
              ...
              >
                <RadioButton
                  v-model="selectedAnswer"
                  :input-id="`boolean-false-${question.questionId}`"
                  :name="`question-${question.questionId}`"
                  value="false"
                  class="hidden me-4"
                />
                <i class="pi pi-times-circle text-rose-600 mr-2" /> No
              </label>
            </div>
          </div>
        </fieldset>
            ...
            <template v-for="value in 5" :key="value">
              <div class="text-center">
                ...
                >
                  <RadioButton
                    v-model="selectedAnswer"
                    :input-id="`scale-${question.questionId}-${value}`"
                    :name="`question-${question.questionId}`"
    ...
    <div class="flex justify-between mt-8">
      ...
      >
        {{ nextButtonText }}
        <i class="pi pi-chevron-right ml-2" />
      </Button>
    </div>
  </div>
</template>
